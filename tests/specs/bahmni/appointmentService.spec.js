import { createService, validateIfServiceExists, deleteServiceByUuid, updateService } from '../../../src/services/openmrs/appointmentService.js';
...
describe('Test Appointment Service API', function () {
    ...
    it('Validate creation of Service with a user having manage service privilege', async function () {
        ...
        expect(response.body).to.have.property('name').to.equal(appointmentServicePayload.name);
    ...
    it('Validation updation of Service Slots with a user having Service availability privilege', async function () {
        ...
        const response = await updateService(serviceUuid, updateServiceAvailabilityPayload);
        
        expect(response.body).to.have.property('appointmentServiceId').to.equal(appointmentServiceId);
        expect(response.body).to.have.property('name').to.equal(updateServiceAvailabilityPayload.name);
    });
